...
Col_Echin<-Cols[1]
LineWidth<-2
plot(seq(to=MaxMA,from=MinMA,length.out=nrow(nspeciesOverTime3)),nspeciesOverTime3[,2],xlim=c(MaxMA,MinMA),type="l",xlab="age(millions of years ago)",ylab="num. of species",col=Col_Arac,lwd=LineWidth)
lines(seq(to=MaxMA,from=MinMA,length.out=nrow(nspeciesOverTime4)),nspeciesOverTime4[,2],col=Col_Echin,lwd=LineWidth)
legend("topleft",legend=c(Taxon3,Taxon4),col=c(Col_Arac,Col_Echin),bty="n",lwd=LineWidth)